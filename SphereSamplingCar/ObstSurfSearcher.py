...
class ObstSurfSearcher(object):
    ...
    def searchObstSurfConfigs( self, freeSamples, obstSamples, minDist ):
        ...
        # Extend points in phi dimension.
        lowFree = [];   upFree = [];
        lowObst = [];   upObst = [];
        for samp in freeSamples:
            lowFree.append(( samp[0], samp[1], samp[2]-self.mCSpace.mMaxDimLens[2] ));
            upFree.append(( samp[0], samp[1], samp[2]+self.mCSpace.mMaxDimLens[2] ));
        freeSamples = freeSamples + lowFree + upFree;
        for samp in obstSamples:
            lowObst.append(( samp[0], samp[1], samp[2]-self.mCSpace.mMaxDimLens[2] ));
            upObst.append(( samp[0], samp[1], samp[2]+self.mCSpace.mMaxDimLens[2] ));
        obstSamples = obstSamples + lowObst + upObst;
    ...
    def loadSamples( self, filename ):
        ...
        for line in file2read:
            samp = line;
            info = samp.split( '\t' );
            newSamp = ( float(info[0]), float(info[1]), float(info[0]) );
            self.mSurfSamples.append( newSamp );
        file2read.close();
...
class CSpace:
    def __init__( self ):
        self.mMaxDimLens = [ 1000, 1000, 1000 ];
...
cspace = CSpace();
searcher = ObstSurfSearcher( Obst, cspace );
