...
class DistSample:
    ...
    def getBoundaryConfigs(self, maxDimLens, num=0):
        ...
        for i in range(0, num):
            ...
            for j in range(0, dim-1):
                ...
                if temp[j] < 0: i-=1;isLegal=False; break;
                elif temp[j] > maxDimLens[j]: i-=1; isLegal=False;  break;
            if( not isLegal ):
                continue;
            temp[2] = (float(temp[2]) / float(length)) * (self.mRadius + 2) + self.mSample[2];
            if temp[2] < 0: temp[2] = (temp[2] + maxDimLens[2]) % maxDimLens[2] ;
            else: temp[2] = temp[2] % maxDimLens[2];

            for j in range( 0, dim ):
                if temp[j] < 0 or temp[j] > maxDimLens[j]:
                    print temp;
    ...
    def isInside(self, position, maxDimLens):
        ...
        for i in range(0, dim):
            ...
            if i == 2 and math.fabs(maxDimLens[i] - deltas[i]) < deltas[i]:
...
class SampleManager:
    ...
    def randomSample( self, num, dim, maxDimLens ):
        for i in range( 0, num ):
            ...
            if( not self.mCollisionMgr.ifCollide( config ) ):
                self.mFreeSamples.append( rnd );
            else:
